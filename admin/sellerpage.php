...
if(isset($_POST['submit'])){
	...
	if($msg==''){
		if(isset($_GET['id']) && $_GET['id']!=''){
			...
			$image=rand(111111111,999999999).'_'.$_FILES['image']['name'];
			move_uploaded_file($_FILES['image']['tmp_name'],'../media/product/'.$image);
			mysqli_query($con,"insert into product(categories_id,name,price,meta_title,meta_desc,meta_keyword,status,image) values('$categories_id','$name','$price','$short_desc','$description','$meta_keyword',1,'$image')");
		}
		header('location:product1.php');
		die();
	}
